      ...
      if (queue.isFull()) {
        ...
        steps.push({
          ...
          state: {
            ...
            capacity: queue.getCapacity(),
            lastAction: 'ENQUEUE',
            statusMessage: `Enqueued value ${cmd.value} into buffer slot [${queue.getRearIndex()}] in O(1) time.`,
          },
          description: `Enqueued value ${cmd.value} at REAR slot [${queue.getRearIndex()}].`,
          a11yMessage: `Enqueued value ${cmd.value} at REAR slot ${queue.getRearIndex()} in O(1) time. Queue contains ${queue.size()} elements.`,
          pointers: getPointers(queue.getFrontIndex(), queue.getRearIndex()),
          codeHighlight: {
            pseudocodeLine: 6,
            typescriptLine: 13,
      ...
      if (queue.isEmpty()) {
        ...
        steps.push({
          ...
          state: {
            ...
            capacity: queue.getCapacity(),
            lastAction: 'DEQUEUE',
            statusMessage: `Dequeued value ${dequeuedVal} from FRONT in O(1) without shifting array elements.`,
          },
          description: `Dequeued value ${dequeuedVal} from FRONT in O(1) time.`,
          a11yMessage: `Dequeued value ${dequeuedVal} from FRONT in O(1) time without shifting. Remaining: ${queue.size()}.`,
          pointers: getPointers(queue.getFrontIndex(), queue.getRearIndex()),
          codeHighlight: {
            pseudocodeLine: 13,
            typescriptLine: 20,
      ...
      if (queue.isEmpty()) {
        ...
        steps.push({
          ...
          state: {
            ...
            statusMessage: `Peeked at FRONT element: ${peekedVal} at slot [${queue.getFrontIndex()}].`,
          },
          description: `Peeked at FRONT slot [${queue.getFrontIndex()}]: ${peekedVal}.`,
          a11yMessage: `Peeked at FRONT slot ${queue.getFrontIndex()} with value ${peekedVal}.`,
          pointers: getPointers(queue.getFrontIndex(), queue.getRearIndex()),
          codeHighlight: {
            pseudocodeLine: 20,
            typescriptLine: 27,
